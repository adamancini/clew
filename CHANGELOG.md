...
### Changed
- Add `.vault/` and `.worktrees/` to `.gitignore`
...
- Updated golang.org/x/term from 0.38.0 to 0.39.0
- Updated golang.org/x/sys from 0.39.0 to 0.40.0 (transitive dependency)

## [0.9.1] - 2026-01-23

### Changed

- Version check workflow now skips validation for non-code changes (GitHub Actions, documentation)
- Allows workflow and documentation updates without requiring clew version bumps
- Go module and code changes still require version bumps
...
- Updated e2e test `TestCLIReaderVsFilesystemReader` to check for `marketplaces` map instead of deprecated `sources` array, aligning with v0.8.0 schema simplification
